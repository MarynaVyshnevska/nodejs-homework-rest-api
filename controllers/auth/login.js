...
const login = async (req, res, next) => {
  ...
  const passwordIsValid = await logedUser.checkPassword(
    password,
    logedUser.password
  );
  if (!passwordIsValid)
    return next(
      new AppError(401, "Email or password is wrong. Not authorized")
    );

  logedUser.password = undefined;

  if (!logedUser.verify) return next(new AppError(403, "Email not verify"));

  const payload = {
    id: logedUser._id,
  };
  const token = jwt.sign(payload, process.env.JWT_SECRET, {
    expiresIn: process.env.JWT_TERMIN,
  });
